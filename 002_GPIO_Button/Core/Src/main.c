...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  	  	  	  ...
	  	  	  	  /*Note that polling mode is not recommended to use*/
	  if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) != GPIO_PIN_RESET )
	  {
		while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0));
		HAL_Delay(100); // because of button bouncing
		counter++;
		if(counter == 4)
			counter = 0;
	  }
	  ...
	  default: HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12 | GPIO_PIN_13 |GPIO_PIN_14| GPIO_PIN_12 , GPIO_PIN_RESET);
	  }
	  HAL_Delay(200);
